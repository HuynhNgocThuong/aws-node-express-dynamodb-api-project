...
import constant from './../src/constant/commonConstant.js';
...
async function create(event) {
  ...
  let userWithThisUsername = await getUserByUsername(newUser.username);
...
async function login(event) {
  ...
  console.log('USER', userWithThisEmail);
  console.log('COUNT', userWithThisEmail.Count);
...
async function update(event) {
  ...
  //Decorate updatedUser and return it (case not update password, email, iamge, bio)
...
async function follow(event) {
  ...
  if (shouldFollow) {
    if (
      ...
      user.followers = Util.getDocumentClient().createSet([
  ...
  if (shouldFollow) {
    if (
      ...
      !authenticatedUser.following.vales.includes(username)
    ) {
      authenticatedUser.following.values.push(username);
    } else {
      authenticatedUser.following = Util.getDocumentClient().createSet([
        username,
      ]);
    }
  } else {
...
async function getFollowedUsers(aUsername) {
  const user = await Util.getDocumentClient()
    .get({
      TableName: usersTable,
      Key: {
        username: aUsername,
...
function mintToken(aUsername) {
  return jwt.sign({ username: aUsername }, constant.tokenSecret, {
    expiresIn: '2 days',
  });
}

function getUserByEmail(aEmail) {
  return Util.getDocumentClient()
    .query({
      TableName: usersTable,
      IndexName: 'email',
...
async function getUserByUsername(aUsername) {
  return await Util.getDocumentClient()
    .get({
      TableName: usersTable,
      Key: {
        username: aUsername,
...
async function authenticateAndGetUser(event) {
  try {
    ...
    console.log(token);
    const decoded = jwt.verify(token, constant.tokenSecret);
